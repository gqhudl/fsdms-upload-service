  ...
  stages {
    ...
    stage('docker build & push image on build docker/build server') {
      ...
      steps {
        ...
        script {
          def REMOVE_FLAG = sh(returnStdout: true, script: "docker container ls -aq --filter name=.*fsdms-upload-ctn.*") != ""
          echo "REMOVE_FLAG: ${REMOVE_FLAG}"
          if(REMOVE_FLAG){
            // sh 'docker container ls -aq --format {{.ID}} --filter name=fsdms-upload-ctn | xargs docker container rm -f'
            // sh 'docker container rm -f $(docker container ls -aq --format {{.ID}} --filter name=.*fsdms-upload-ctn.*)'
            sh 'docker container rm -f $(docker container ls -aq --filter name=.*fsdms-upload-ctn.*)'
          }
        }

        // docker rmi old images before build: remove only there are images found
        script {
          def REMOVE_FLAG = sh(returnStdout: true, script: "docker image ls -q *${DOCKHUB_USERNAME}/fsdms-upload-srv*") != ""
          echo "REMOVE_FLAG: ${REMOVE_FLAG}"
          if(REMOVE_FLAG){
            // sh 'docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/fsdms-upload-srv* | xargs docker image rm -f'
            // sh 'docker image rm -f $(docker image ls --format {{.ID}} *${DOCKHUB_USERNAME}/fsdms-upload-srv*)'
            sh 'docker image rm -f $(docker image ls -q *${DOCKHUB_USERNAME}/fsdms-upload-srv*)'
          }
        }
        ...
        // script {
          ...
          //   def customImage = docker.build("${DOCKHUB_USERNAME}/fsdms-upload-srv:latest", '-f Dockerfile .')
        ...
        withCredentials([usernamePassword(credentialsId: "DockerHub_${DOCKHUB_USERNAME}", usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
          ...
          sh 'docker image build -t ${DOCKHUB_USERNAME}/fsdms-upload-srv:latest .'
          sh 'docker push ${DOCKHUB_USERNAME}/fsdms-upload-srv:latest'
        }
      }
    }
    ...
    stage('docker pull image and docker run image on docker/deploy server') {
      ...
      steps {
        ...
        // sh 'docker pull ${DOCKHUB_USERNAME}/fsdms-upload-srv'

        // docker run images
        sh 'docker run -d -p 9050:9050 -v fsdms-data:/fsdms-data --network fsdms-net --name fsdms-upload-ctn ${DOCKHUB_USERNAME}/fsdms-upload-srv -Xms128m -Xmx128m'
      }
    }
